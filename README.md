shadowsocks-heroku-winxp
==================

shadowsocks-heroku-winxp is a client for shadowsocks-heroku. It is compatible with node < v6, so it can be used in win xp.

`locals.js` and `config.json` are for local client and other files are as the same as the shadowsocks-heoku (https://github.com/mrluanma/shadowsocks-heroku).
